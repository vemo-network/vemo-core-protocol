...
On version 1, the core framework of Vemo is built around three core contracts: Voucher.sol, ERC6551Account.sol. The Voucher contract functions as the manager/registry of the ERC6551 Collection (nft), allowing user to not only create voucher collection but also represent as a portal to create ERC6551Account(s), the ERC6551Account contract is responsible for containing the token assets. Each ERC6551Account (token bound account) is linked 1:1 with a ERC721 token (nft), employing mechanisms that allows rewards to be claimed over a fixed period or linear hours/days/months. This implementation enables vesting position(s) trable thru the ERC721 ownership while maintaining the vesting logic.
...
-- | --
Voucher.sol | The code defines a smart contract, Voucher, responsible for managing the distribution of NFT token and token bound account (in the form of ERC6551Account) to managing the vestings positions. It also serves as a portal, enabling users to create NFT:ERC6551 in batch, specify vesting schedule and request common information.
.sol | The code defines V3FactoryOwner, a contract serving as the owner of the Uniswap v3 factory, allowing an admin (expected to be Uniswap Governance) to manage fee settings on pools and the factory itself. It enables a public function for collecting protocol fees from pools in exchange for a specified token, aiming to create a competitive market for fee collection.
