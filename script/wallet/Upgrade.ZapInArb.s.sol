...
contract DeployZapperARB is Script {
    ...
    function run() public {
        ...
        vePendleZapIn vetermImplementation = new vePendleZapIn{salt: bytes32(salt)}();

        UUPSProxy proxyVePendleZapIn = new UUPSProxy{salt: bytes32(salt)}(
            address(vetermImplementation),
            abi.encodeWithSelector(vePendleZapIn.initialize.selector, owner, walletFactoryProxy, owner)
        );
        console.log("zapin vependle address proxy ", address(proxyVePendleZapIn));
        vm.stopBroadcast();
    }
